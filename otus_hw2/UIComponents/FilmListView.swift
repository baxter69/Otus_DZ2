...
struct CategoryPicker: View {
    ...
    var body: some View {
        Picker("Категория", selection: $selectedCategory) {
            Text("⭐ Популярное").tag(FilmsAPI.ModelType_apiV22FilmsCollectionsGet.topPopularAll)
            Text("🎞️ Топ фильмов").tag(FilmsAPI.ModelType_apiV22FilmsCollectionsGet.top250Movies)
            Text("📺 Топ сериалов").tag(FilmsAPI.ModelType_apiV22FilmsCollectionsGet.top250TvShows)
        }
